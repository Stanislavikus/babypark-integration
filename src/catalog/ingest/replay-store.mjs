...
export class ReplayStore {
    ...
    try {
      ...
      db.exec(`
        ...
        CREATE TABLE receipts (
          ...
          CHECK((status='pending' AND ack_json IS NULL) OR
                (status='acked' AND ack_json IS NOT NULL))
        );
    ...
    return transact(this.db, () => {
      ...
      if (row) {
        ...
        return row.status === 'acked'
          ? { status: 'ACKED', ack: JSON.parse(row.ack_json) }
          : { status: 'PENDING' };
      }
  ...
  complete(key, ack) {
    ...
    return transact(this.db, () => {
      ...
      if (row.status === 'acked') {
        if (row.ack_json !== ackJson) {
          fail('INGEST_REPLAY_ACK_CONFLICT', 'Committed ACK cannot change');
        }
        return JSON.parse(row.ack_json);
      }
      this.db.prepare(
        "UPDATE receipts SET status='acked', ack_json=? " +
        'WHERE kid=? AND run_id=? AND layer=? AND seq=?'
      ).run(ackJson, kid, runId, layer, seq);
