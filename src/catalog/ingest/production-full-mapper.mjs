...
import { validateFullRecords, FullRecordError } from './full-record-v1.mjs';
...
export function mapProductionFullRecords(writer, records, identityStore) {
  ...
  for(const r of ordered){
    if(r.type==='brand')writer.writeBrand({brand_id:dimensionId(r.type,r.provider,r.native_brand_id),name:r.name,provenance_json:provenance(r.provider,r.native_brand_id,r.provenance)});
      ...
      for(const v of r.variants){const variant_id=variants.get(v.native_variant_id), n=normalizeSku(v.sku);writer.writeVariant({variant_id,product_id:p.product_id,sku:v.sku,sku_key:n.sku_key,gtin:v.gtin??null,is_default:+v.is_default,options_json:canonical(v.options??{}),updated_at:v.updated_at});if(v.offer)writer.writeOffer({variant_id,current_minor:v.offer.current_minor,regular_minor:v.offer.regular_minor??null,currency:v.offer.currency,on_sale:+v.offer.on_sale,commercial_availability:v.offer.commercial_availability,tax_included:v.offer.tax_included===undefined?null:+v.offer.tax_included,valid_from:v.offer.valid_from??null,valid_to:v.offer.valid_to??null,source_updated_at:v.offer.source_updated_at??null});for(const s of v.stock)writer.writeStoreStock({variant_id,store_id:dimensionId('store',r.provider,s.store_native_id),quantity:s.quantity,source_updated_at:s.source_updated_at??null});for(const a of v.attributes){const d=writer.readAttributeDefinition(dimensionId('attribute_definition',r.provider,a.native_attribute_id));if(!d)fail('FULL_MAPPER_REFERENCE_MISSING','attribute definition missing');if(!checkValue(d.type,a.value))fail('FULL_RECORD_INVALID','attribute value type mismatch');writer.writeAttributeValue({owner_type:'VARIANT',owner_id:variant_id,attribute_id:d.attribute_id,value_json:canonical(a.value)});}}
      ...
      for(const a of r.attributes){const d=writer.readAttributeDefinition(dimensionId('attribute_definition',r.provider,a.native_attribute_id));if(!d)fail('FULL_MAPPER_REFERENCE_MISSING','attribute definition missing');if(!checkValue(d.type,a.value))fail('FULL_RECORD_INVALID','attribute value type mismatch');writer.writeAttributeValue({owner_type:'PRODUCT',owner_id:p.product_id,attribute_id:d.attribute_id,value_json:canonical(a.value)});}
      for(const i of r.images)writer.writeImage({image_id:imageId(r.provider,r.native_product_id,i.native_image_id),product_id:p.product_id,variant_id:i.variant_native_id?variants.get(i.variant_native_id):null,url:i.url,role:i.role??null,position:i.position??0,metadata_json:provenance(r.provider,i.native_image_id,{native_product_id:r.native_product_id,...(i.metadata??{})})});
    } else if(r.type==='kit_component'){
      const p=identityStore.lookupProductBySource({provider:r.provider,nativeProductId:r.kit_native_product_id});const v=identityStore.lookupVariantBySource({provider:r.provider,nativeVariantId:r.component_native_variant_id});if(!p||!v)fail('FULL_MAPPER_REFERENCE_MISSING','KIT identity reference missing');writer.writeKitComponent({kit_product_id:p.product_id,component_variant_id:v.variant_id,quantity:r.quantity,discount_minor:r.discount_minor??null,mutable:+(r.mutable??false),metadata_json:canonical(r.metadata??{})});
    }
  }
