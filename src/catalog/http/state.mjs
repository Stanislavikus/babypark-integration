...
export function readCatalogState(reader, { ingestEnabled }) {
  const blockers = ingestEnabled ? [] : ['INGEST_DISABLED'];
  try {
    ...
    return { schema: 'bp.catalog.state/1', state: 'CURRENT', accepting_ingest: ingestEnabled, blockers,
    ...
    return { schema: 'bp.catalog.state/1', state: 'BOOTSTRAP', accepting_ingest: ingestEnabled, blockers,
