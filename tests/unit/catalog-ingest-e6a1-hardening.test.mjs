...
]) test(`semantic certification rejects ${name} before ACCEPTED`, t => { const fixture = semanticFixture(t, `semantic-${name.replaceAll(' ', '-')}`); mutate(fixture); assert.throws(() => prepareProductionCertification({ db: fixture.builder.db, identityStore: fixture.identity }), code('FULL_SEMANTIC_CERTIFICATION_FAILED')); assert.equal(fixture.builder.db.prepare("SELECT count(*) n FROM ingest_runs WHERE status='ACCEPTED'").get().n, 0); });
...
test('dependency mismatch rejects before ACCEPTED', t => { const fixture = semanticFixture(t, 'dependency-mismatch'); fixture.builder.db.prepare("UPDATE catalog_meta SET dependency_fingerprint='wrong'").run(); assert.throws(() => prepareProductionCertification({ db: fixture.builder.db, identityStore: fixture.identity }), code('FULL_DEPENDENCY_MISMATCH')); assert.equal(fixture.builder.db.prepare("SELECT count(*) n FROM ingest_runs WHERE status='ACCEPTED'").get().n, 0); });
